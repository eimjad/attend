...
<div class="attend-wrapper">
  ...
  <div id="user-info" style="margin-bottom:1em; display:none;">
    <p><strong>Name:</strong> <span id="user-name"></span><br>
  ...
  <div id="attend-controls" style="display:auto;">
    ...
    <textarea id="attend-note" rows="2" placeholder="Add a note"></textarea>
  ...
  (function () {
    ...
    function postPayload(payload) {
      ...
      .then(json => {
        ...
        const statusEl = qs('#attend-status');
        ...
        if (json.status === 'success') {
          ...
          // ✅ Disable controls after successful submit
          ...
          // ✅ Determine which action message to show
          const actionText = (json.action === 'in') 
            ? 'Clock In' 
            : (json.action === 'out') 
            ? 'Clock Out' 
            : 'Attendance';
          ...
          if (statusEl) {
            ...
            statusEl.innerText = `${actionText} recorded on ${dayName}, ${dateStr}, ${timeStr}.`;
          }
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      checkLocation(allowed, officeLat, officeLng, function (ok) {
        ...
        if (ok) {
          ...
          // Outside radius
          if (noteLabel) noteLabel.style.display = 'block';
          if (note) note.style.display = 'block';
          const hasText = note && note.value.trim().length > 0;
          if (btnIn) btnIn.disabled = !hasText;
          if (btnOut) btnOut.disabled = !hasText;

          if (note && !note.dataset.listenerAdded) {
            note.addEventListener('input', function () {
              const textOk = this.value.trim().length > 0;
              if (btnIn) btnIn.disabled = !textOk;
              if (btnOut) btnOut.disabled = !textOk;
            });
            note.dataset.listenerAdded = 'true';
          }
        }
      });
    });
